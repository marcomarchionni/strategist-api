...
class DividendServiceImplTest {
  ...
  @Mock DividendAccessService dividendAccessService;
  ...
  void setup() {
    ...
    dividendService =
        new DividendServiceImpl(dividendAccessService, strategyAccessService, dividendMapper);
  }
  ...
  void findByParams() {
    ...
    when(dividendAccessService.findByParams(
        ...
        .thenReturn(userDividends);

    // execute method
    List<DividendSummary> foundDividends = dividendService.findByFilter(dividendFind);
  ...
  void updateStrategyId() {
    ...
    StrategyAssign dividendUpdate =
        StrategyAssign.builder().id(dividend.getId()).strategyId(strategy.getId()).build();
    ...
    when(dividendAccessService.save(any(Dividend.class)))
  ...
  void updateStrategyIdNullSuccess() {
    // setup test data
    StrategyAssign dividendUpdate =
        StrategyAssign.builder().id(dividend.getId()).strategyId(null).build();
  ...
  void updateStrategyIdException() {
    ...
    // execute method
    assertThrows(
        EntityNotFoundException.class, () -> dividendService.updateStrategyId(dividendUpdate));
  }
  ...
  void updateDividends() {
    ...
    // new dividends
    List<Dividend> newDividends =
        List.of(getFDXOpenDividend(), getNKEClosedDividend(), getEBAYClosedDividend());
    ...
    when(dividendAccessService.existsByActionId(getFDXOpenDividend().getActionId()))
        .thenReturn(false);
    when(dividendAccessService.saveAll(anyList()))
        .thenAnswer(invocation -> invocation.getArgument(0));
    when(dividendAccessService.saveAll(anyList()))
        .thenAnswer(invocation -> invocation.getArgument(0));
