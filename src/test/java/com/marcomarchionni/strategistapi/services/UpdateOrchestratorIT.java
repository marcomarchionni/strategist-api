...
public class UpdateOrchestratorIT {
  ...
  @Autowired UpdateOrchestrator updateOrchestrator;
  ...
  void updateFromFileEmptyDbTest() throws IOException {
    ...
    assertTrue(
        flexStatements.stream().allMatch(fs -> fs.getAccountId().equals(user.getAccountId())));

    // Assert dividends' openClosed is not null
    Optional<Dividend> result =
        dividends.stream().filter(d -> d.getOpenClosed() == null).findFirst();
  ...
  void updateFromFilePopulatedDbTest() throws IOException {
    ...
    long countTradesWithStrategyBefore =
        existingTrades.stream().filter(trade -> trade.getStrategy() != null).count();
    ...
    long countTradesWithStrategyAfter =
        updatedTrades.stream().filter(trade -> trade.getStrategy() != null).count();

    List<Position> updatedPositions = positionRepository.findAll();
    long countPositionsWithStrategyAfter =
        updatedPositions.stream().filter(position -> position.getStrategy() != null).count();
    ...
    Set<Long> existingFlexIds =
        existingFlexStatements.stream().map(FlexStatement::getId).collect(Collectors.toSet());
    FlexStatement addedFlexStatement =
        updatedFlexStatements.stream()
            .filter(fs -> !existingFlexIds.contains(fs.getId()))
            .toList()
            .get(0);
    ...
    assertEquals(
        existingTrades.size() + report.getTrades().getAdded().size(), updatedTrades.size());
    assertEquals(countTradesWithStrategyBefore, countTradesWithStrategyAfter);

    // assert positions are updated
    assertEquals(
        report.getPositions().getAdded().size() + report.getPositions().getMerged().size(),
    ...
    assertEquals(
        existingDividends.size() + report.getDividends().getAdded().size(),
        updatedDividends.size());
    if (!report.getDividends().getMerged().isEmpty()) {
      DividendSummary reportedMergedDividend = report.getDividends().getMerged().get(0);
      Dividend dividendBeforeMerge =
          existingDividends.stream()
              .filter(dividend -> dividend.getId().equals(reportedMergedDividend.getId()))
              .findFirst()
              .orElse(null);
      assertNotNull(dividendBeforeMerge);
      Dividend dividendAfterMerge =
          updatedDividends.stream()
              .filter(dividend -> dividend.getId().equals(reportedMergedDividend.getId()))
              .findFirst()
              .orElse(null);
