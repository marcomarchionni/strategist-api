...
class FilterParserImplTest {
  ...
  static Stream<Arguments> provideFilters() {
    return Stream.of(
        Arguments.of(
            "startswith(tolower(name),'b')", List.of(new ParsedFilter("startswith", "name", "b"))),
        Arguments.of(
            "substringof('test', tolower(description))",
            List.of(new ParsedFilter("substringof", "description", "test"))),
        Arguments.of(
            "endswith(tolower(account),'123')",
            List.of(new ParsedFilter("endswith", "account", "123"))),
        Arguments.of(
            "(startswith(tolower(name),'a'))",
            List.of(new ParsedFilter("startswith", "name", "a"))),
        Arguments.of("(tolower(name) eq '')", List.of(new ParsedFilter("eq", "name", ""))),
        Arguments.of(
            "not startswith(tolower(name),'e')",
            List.of(new ParsedFilter("not startswith", "name", "e"))),
        Arguments.of(
            "(createdAt gt datetime'2024-10-10T21:59:59.999Z')",
            List.of(new ParsedFilter("gt", "createdAt", "datetime2024-10-10T21:59:59.999Z"))),
        Arguments.of(
            "((createdAt gt datetime'2024-10-01T21:59:59.999Z') and (createdAt lt "
