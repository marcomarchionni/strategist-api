...
class PagingUtilTest {
  ...
  private static Stream<Arguments> providePagingData() {
    return Stream.of(
        ...
        Arguments.of(
            20, 10, "name asc", 2, 10, true, Sort.Direction.ASC, "name"), // Ascending order
        Arguments.of(
            30, 10, "createdAt desc", 3, 10, true, Sort.Direction.DESC, "createdAt"), // Descending
        // order
  ...
  void testCreatePageable(
    ...
    if (isSorted) {
      ...
      assertEquals(
          direction,
          Objects.requireNonNull(pageable.getSort().getOrderFor(sortField)).getDirection());
    } else {
