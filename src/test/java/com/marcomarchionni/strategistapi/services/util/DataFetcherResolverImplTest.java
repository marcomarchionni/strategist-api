...
class DataFetcherResolverImplTest {
  ...
  @Mock FileDataFetcher fileDataFetcher;

  @Mock ServerDataFetcher serverDataFetcher;

  @Mock SampleDataFetcher sampleDataFetcher;
  ...
  void setUp() {
    dataFetcherResolver =
        new DataFetcherResolverImpl(fileDataFetcher, serverDataFetcher, sampleDataFetcher);
  }

  @Test
  void resolve() {
    DataFetcher dataFetcher = dataFetcherResolver.resolve(UpdateContext.SourceType.FILE);
    assertNotNull(dataFetcher);
    assertInstanceOf(FileDataFetcher.class, dataFetcher);
    dataFetcher = dataFetcherResolver.resolve(UpdateContext.SourceType.SERVER);
    assertNotNull(dataFetcher);
