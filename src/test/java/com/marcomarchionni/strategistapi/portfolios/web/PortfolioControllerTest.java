...
class PortfolioControllerTest {
  ...
  void findPortfolios() throws Exception {
    ...
    List<PortfolioSummary> portfolioSummaries =
        getSamplePortfolios().stream()
            .peek(portfolio -> portfolio.setAccountId(accountId))
            .map(portfolioMapper::portfolioToPortfolioSummary)
            .toList();
    ApiResponse<PortfolioSummary> response =
        ApiResponse.<PortfolioSummary>builder()
  ...
  void findPortfoliosWithCount() throws Exception {
    ...
    // Execute test
    mockMvc
        .perform(get("/portfolios?skip=0&top=10"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.result", hasSize(portfolioSummaries.size())))
        .andExpect(jsonPath("$.count", is(portfolioSummaries.size())));
  ...
  void findPortfolioException() throws Exception {
    ...
    // setup mock behavior
    when(portfolioService.findById(unknownPortfolioId))
        .thenThrow(new EntityNotFoundException(Portfolio.class, unknownPortfolioId));

    // Execute test
    mockMvc
        .perform(get("/portfolios/{id}", unknownPortfolioId))
        .andDo(print())
