...
class PortfolioServiceImplTest {
  ...
  @Mock Specification<Portfolio> spec;
  @Mock SimplePortfolioSpecification portfolioSpecification;
  ...
  void setup() {
    ...
    portfolioService =
        new PortfolioServiceImpl(
            userService, portfolioMapper, portfolioRepository, portfolioSpecification);
  ...
  void findAllWithCount() {
    ...
    FindAllReq findReq = FindAllReq.builder().skip(0).top(10).build();
    ...
    when(portfolioRepository.findAll(
            ArgumentMatchers.<Specification<Portfolio>>any(), any(Pageable.class)))
    ...
    when(portfolioSpecification.buildSpecification(accountId, null, null, null, null))
        .thenReturn(spec);
  ...
  void findByIdException() {
    ...
    assertThrows(
        EntityNotFoundException.class, () -> portfolioService.findById(unknownPortfolioId));
  }
  ...
  void createPortfolioSuccess() {
    ...
    // Setup mocks
    when(portfolioRepository.findByAccountIdAndName(accountId, portfolioName))
  ...
  void updatePortfolioSuccess() {
    ...
    when(portfolioRepository.findById(portfolioId)).thenReturn(Optional.of(userPortfolio));
    when(userService.getUserAccountId()).thenReturn(accountId);
    when(portfolioRepository.findByAccountIdAndName(accountId, "NewName"))
        .thenReturn(Optional.empty());
    when(portfolioRepository.save(any(Portfolio.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
  ...
  void deleteByIdException() {
    ...
    // execute service and verify exception
    assertThrows(
        EntityNotFoundException.class, () -> portfolioService.deleteById(unknownPortfolioId));
  }
}
