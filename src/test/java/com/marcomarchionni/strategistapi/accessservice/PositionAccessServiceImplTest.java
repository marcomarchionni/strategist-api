...
class PositionAccessServiceImplTest {
  ...
  void setup() {
    ...
    positionAccessService =
        new PositionAccessServiceImpl(positionRepository, userService, accountIdValidator);
  ...
  void deleteAllException() {
    ...
    assertThrows(
        InvalidUserDataException.class, () -> positionAccessService.deleteAll(expectedPositions));
  }
  ...
  void saveAllEx() {
    expectedPositions.get(0).setAccountId("U2222222");
    assertThrows(
        InvalidUserDataException.class, () -> positionAccessService.saveAll(expectedPositions));
  }

  @Test
  void findByParams() {
    when(positionRepository.findByParams("U1111111", true, null, null))
  ...
  @Test
  void findBySymbol() {
    when(positionRepository.findByAccountIdAndSymbol("U1111111", "AAPL"))
        .thenReturn(Optional.ofNullable(expectedPosition));
  ...
  @Test
  void findById() {
    when(positionRepository.findByIdAndAccountId(1L, "U1111111"))
        .thenReturn(Optional.ofNullable(expectedPosition));
  ...
  void saveEx() {
    ...
    assertThrows(
        InvalidUserDataException.class, () -> positionAccessService.save(expectedPosition));
  }
}
