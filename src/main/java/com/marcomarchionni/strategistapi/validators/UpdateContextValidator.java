...
public class UpdateContextValidator
    implements ConstraintValidator<ValidUpdateContext, UpdateContext> {
  @Override
  ...
  private boolean checkFileParameter(UpdateContext dto, ConstraintValidatorContext context) {
    if (dto.getFile() == null) {
      ...
      context
          .buildConstraintViolationWithTemplate("File parameter is required")
  ...
  private boolean checkQueryIdAndTokenParameters(
      UpdateContext dto, ConstraintValidatorContext context) {
    boolean valid = true;
    if (dto.getQueryId() == null) {
      context.disableDefaultConstraintViolation();
      context
          .buildConstraintViolationWithTemplate("QueryId parameter is required")
          .addConstraintViolation();
      valid = false;
    }
    if (dto.getToken() == null) {
      context.disableDefaultConstraintViolation();
      context
          .buildConstraintViolationWithTemplate("Token parameter is required")
          .addConstraintViolation();
      valid = false;
    }
    return valid;
