...
public class AccountIdConstraintValidator
    implements ConstraintValidator<ValidAccountId, UpdateDto> {
  ...
  public boolean isValid(UpdateDto dto, ConstraintValidatorContext constraintValidatorContext) {
    try {
      ...
      boolean validOpenPositions =
          dto.getPositions().stream()
              .allMatch(position -> position.getAccountId().equals(accountId));
      boolean validTrades =
          dto.getTrades().stream().allMatch(trade -> trade.getAccountId().equals(accountId));
      boolean validDividends =
          dto.getDividends().stream()
