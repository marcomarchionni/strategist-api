...
public class ResponseParserImpl implements ResponseParser {
  ...
  private Predicate<FlexQueryResponseDto.ChangeInDividendAccrual> isValidClosedDividend() {
    return cd ->
        cd.getLevelOfDetail().equalsIgnoreCase("DETAIL")
            && cd.getCode().equalsIgnoreCase("Re")
  ...
  private List<Dividend> getDividends(FlexQueryResponseDto dto) {
    var closedDividendsStream =
        dto.nullSafeGetChangeInDividendAccruals().stream()
            .filter(isValidClosedDividend())
            .map(dividendMapper::toClosedDividend);

    var openDividendsStream =
        dto.nullSafeGetOpenDividendAccruals().stream().map(dividendMapper::toOpenDividend);
