  ...
  public Specification<Portfolio> fromFilter(String filter, String accountId) {
    return (root, query, criteriaBuilder) -> {
      ...
      List<Predicate> predicates =
          parsedFilters.stream()
              .map(
                  parsedFilter -> {
                    Predicate predicate =
                        predicateBuilder.build(root, criteriaBuilder, parsedFilter);
                    if (predicate == null) {
                      throw new FilterParsingException(
                          "Unsupported filter expression: " + parsedFilter.expression());
                    }
                    return predicate;
                  })
              .toList();
      ...
      return criteriaBuilder.and(
          accountPredicate, criteriaBuilder.and(predicates.toArray(new Predicate[0])));
    };
  }
}
